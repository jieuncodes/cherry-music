...
export const fetchTopTracks = async (req, res) => {
  try {
    const response = await fetch(`${LAST_FM_BASE_URL}/?method=artist.getTopTracks&artist=${encodeURIComponent(ARTIST_NAME)}&api_key=${process.env.LAST_FM_API_KEY}&format=json`);
    if (!response.ok) {
