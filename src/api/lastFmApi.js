...
export const getTrackInfo = async ({artist, trackTitle}) => {
  try {
    const response = await fetch(
      `${LAST_FM_BASE_URL}?method=track.getInfo&api_key=${process.env.LAST_FM_API_KEY}&artist=${artist}&track=${trackTitle}&format=json`)
